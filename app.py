    ...
    if geo_estados:
        folium.GeoJson(
            ...
            style_function=lambda f: f.get("properties", {}).get("style", {
                "color": "#000000", "weight": 5, "dashArray": "0", "fillOpacity": 0
            }),
    ...
    for _, row in df.iterrows():
        ...
        if geojson and "features" in geojson:
            folium.GeoJson(
                ...
                tooltip=f"{row['Distribuidor']} ({cidade})"
            ).add_to(mapa)
        else:
            try:
                ...
                folium.Marker(
                    ...
                    popup=f"{row['Distribuidor']} ({cidade})"
                ).add_to(mapa)
            except:
                continue
    ...
    if nivel_cookie == "editor":
        with st.expander("✏️ Editar"):
            if not st.session_state.df.empty:
                ...
                estado_edit = st.selectbox("Estado", sorted(st.session_state.df["Estado"].unique()),
